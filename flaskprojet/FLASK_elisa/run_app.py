...
def recherche_sql(donnees):
    ...
    """
    parametre0, parametre1, parametre2 = "", "", ""
    if donnees["Nom"] != "":
        parametre0 = "%" + donnees["Nom"] + "%"
    if donnees["singles"] != "":
        parametre1 = "%" + donnees["singles"] + "%"
    if donnees["singles2"] != "":
        parametre2 = "%" + donnees["singles2"] + "%"

    parametres = (parametre0, parametre1, parametre2)
    connexion = sqlite3.connect("bdd/artistes_connus.db")
    curseur = connexion.cursor()
    requete_sql = """
    SELECT *
...
def ajoute_enregistrement(donnees):
    ...
    curseur = connexion.cursor()
    requete_sql = """
    INSERT INTO artistes_famous(id, Nom, nombre_abos, singles, singles2) VALUES (?,?,?,?,?);"""
    resultat = curseur.execute(requete_sql, parametres)
    connexion.commit()
    connexion.close()
    return True
...
def modification_ligne():
    result = (
        request.form
    )  # Récupération des informations en provenance du POST: C'est un dictionnaire
    liste_artistes = recherche_sql(result)
    return render_template(
        "modification.html",
        Nom=result["Nom"],
        singles=result["singles"],
        singles2=result["singles2"],
...
def valide_saisie():
    ...
    index = index_max()
    ajoute_enregistrement(
        result
    )  # Créé l'enregistrement avec le nouvel id et les données saisies
    artistes = (
        lire_base()
    )  # Interroge la base pour récupérer la liste des labos avant de l'afficher
    return render_template("lecture.html", artistes=artistes)
